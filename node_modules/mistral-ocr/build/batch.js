...
import { buildMarkdownFromOcrResponse, DEFAULT_MODEL, DEFAULT_PAGE_SEPARATOR, extractImagesFromOcrResponse, writeExtractedImages, } from './convert.js';
...
function createMistralClient(apiKey = process.env.MISTRAL_API_KEY) {
    if (!apiKey) {
        throw new Error('MISTRAL_API_KEY is required. Pass apiKey or set the environment variable.');
    }
    return new Mistral({ apiKey });
}
async function normalizePdfInput(input) {
...
function createImagesBySource(images) {
...
function parseOcrBatchOutput(raw, index) {
...
async function downloadBatchOutputs(client, job) {
...
function createUniqueOutputBaseName(fileName, seen) {
    const baseName = path.basename(fileName, path.extname(fileName)) || 'document';
    const count = seen.get(baseName) ?? 0;
    seen.set(baseName, count + 1);
    return count === 0 ? baseName : `${baseName}-${count + 1}`;
}
async function writeBatchEntryOutput(output, file, outputBaseName, options) {
    if (!output.response) {
        return {
    ...
    const markdownPath = outputDir && options.writeMarkdown !== false ? path.join(outputDir, `${outputBaseName}.md`) : undefined;
    const docxPath = outputDir && options.generateDocx !== false ? path.join(outputDir, `${outputBaseName}.docx`) : undefined;
    const imageOutputDir = outputDir && options.writeImages !== false ? path.join(outputDir, `${outputBaseName}-images`) : undefined;
    ...
    const markdown = buildMarkdownFromOcrResponse(output.response, imageReplacements, options.pageSeparator ?? DEFAULT_PAGE_SEPARATOR);
    if (markdownPath) {
...
export async function createOcrBatch(inputs, options = {}) {
    if (inputs.length === 0) {
...
export async function waitForOcrBatch(jobId, options = {}) {
    const logger = getLogger(options.logger);
    const client = options.client ?? createMistralClient(options.apiKey);
...
export async function convertPdfBatch(inputs, options = {}) {
    ...
    const seenOutputBaseNames = new Map();
    const entries = [];
