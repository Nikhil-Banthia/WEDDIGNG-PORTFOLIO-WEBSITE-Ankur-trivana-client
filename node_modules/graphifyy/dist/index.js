...
var DEFAULT_GRAPHIFY_STATE_DIR, LEGACY_GRAPHIFY_STATE_DIR, NEXT_GRAPHIFY_STATE_DIR;
var init_paths = __esm({
  "src/paths.ts"() {
    DEFAULT_GRAPHIFY_STATE_DIR = ".graphify";
    LEGACY_GRAPHIFY_STATE_DIR = "graphify-out";
    NEXT_GRAPHIFY_STATE_DIR = DEFAULT_GRAPHIFY_STATE_DIR;
  }
});
...
function buildMerge(newChunks, options) {
  const graphPath = resolve8(options?.graphPath ?? ".graphify/graph.json");
  const base = [];
  let existingNodeCount = 0;
  if (existsSync6(graphPath)) {
    const existingGraph = loadGraphFromData(JSON.parse(readFileSync7(graphPath, "utf-8")));
    const existingNodes = [];
...
import { join as join10, resolve as resolve11, extname as extname5, basename as basename5, relative as relative4, sep as sep2, dirname as dirname7 } from "path";
...
function looksLikePaper(filePath) {
  try {
    const text = readFileSync9(filePath, "utf-8").slice(0, 3e3);
...
function loadGraphifyignore(root) {
  const patterns = [];
  let current = resolve11(root);
  while (true) {
    const ignoreFile = join10(current, ".graphifyignore");
    if (existsSync9(ignoreFile)) {
      for (let line of readFileSync9(ignoreFile, "utf-8").split("\n")) {
        line = line.trim();
    ...
    if (existsSync9(join10(current, ".git"))) {
      break;
    }
...
function isIgnored(filePath, root, patterns) {
  ...
  let rel3;
  try {
    rel3 = relative4(root, filePath).replace(/\\/g, "/");
  } catch {
    return false;
  }
  const parts = rel3.split("/");
  for (const pattern of patterns) {
    const p = pattern.replace(/^\/+|\/+$/g, "");
    if (!p) continue;
    if (matchGlob(rel3, p)) return true;
    if (matchGlob(basename5(filePath), p)) return true;
...
function resolveCandidateFiles(rootResolved, candidateRoot, candidateFiles, followSymlinks) {
  ...
  for (const file of candidateFiles) {
    const fullPath = resolve11(candidateRoot, file);
...
function detect(root, options) {
  ...
  const rootResolved = resolve11(root);
  ...
  const allFiles = options?.candidateFiles !== void 0 && options.candidateFiles !== null ? resolveCandidateFiles(rootResolved, options.candidateRoot ?? rootResolved, options.candidateFiles, followSymlinks) : [
    ...
    ...existsSync9(memoryDir) ? walkDir(memoryDir, rootResolved, ignorePatterns, followSymlinks, true) : []
  ];
  const seen = /* @__PURE__ */ new Set();
  for (const p of allFiles) {
    if (seen.has(p)) continue;
    seen.add(p);
    const inMemory = existsSync9(memoryDir) && p.startsWith(memoryDir);
    if (!inMemory) {
      if (basename5(p).startsWith(".")) continue;
    ...
    if (!ftype) continue;
    if (OFFICE_EXTENSIONS.has(extname5(p).toLowerCase())) {
...
function loadManifest(manifestPath = defaultManifestPath()) {
  try {
    return JSON.parse(readFileSync9(manifestPath, "utf-8"));
  } catch {
    return {};
  }
}
...
function toJson(G, communities, outputPath, communityLabelsOrOptions) {
  ...
  if (!forceWrite) {
    try {
      const existing = JSON.parse(readFileSync10(outputPath, "utf-8"));
  ...
  writeFileSync8(outputPath, JSON.stringify(output, null, 2), "utf-8");
}
function toCypher(G, outputPath) {
  ...
  G.forEachEdge((_edge, data, u, v) => {
    const rel3 = (data.relation ?? "RELATES_TO").toUpperCase().replace(/[^A-Za-z0-9_]/g, "_");
    const conf = cypherEscape(data.confidence ?? "EXTRACTED");
    const uEsc = cypherEscape(u);
    const vEsc = cypherEscape(v);
    lines.push(
      `MATCH (a {id: '${uEsc}'}), (b {id: '${vEsc}'}) MERGE (a)-[:${rel3} {confidence: '${conf}'}]->(b);`
    );
  });
...
${hyperedgeScript(hyperedgesJson)}
  ...
  writeFileSync8(outputPath, html, "utf-8");
}
...
function toSvg(G, communities, outputPath, communityLabelsOrOptions, figsize = [20, 14]) {
  ...
  svgParts.push("</svg>");
  writeFileSync8(outputPath, svgParts.join("\n"), "utf-8");
}
function toCanvas(G, communities, outputPath, communityLabelsOrOptions, nodeFilenames) {
  ...
  if (!providedNodeFilenames) {
    ...
    G.forEachNode((nodeId, data) => {
      const base = safeName2(data.label ?? nodeId);
  ...
  for (let idx = 0; idx < sortedCids.length; idx++) {
    ...
    for (let mIdx = 0; mIdx < sortedMembers.length; mIdx++) {
      ...
      const fname = filenameMap.get(nodeId) ?? safeName2(G.getNodeAttribute(nodeId, "label") ?? nodeId);
      canvasNodes.push({
        id: `n_${nodeId}`,
        type: "file",
  ...
  writeFileSync8(outputPath, JSON.stringify(canvasData, null, 2), "utf-8");
}
...
import { extname as extname6, join as join12, resolve as resolve12 } from "path";
...
function fileHash2(filePath) {
  ...
  const raw = readFileSync11(filePath);
  const content = extname6(filePath).toLowerCase() === ".md" ? bodyContent(raw) : raw;
...
function cacheDir(root = ".", options = {}) {
  ...
  const d = namespace ? join12(resolveGraphifyPaths({ root }).cacheDir, namespace) : resolveGraphifyPaths({ root }).cacheDir;
...
function loadCached(filePath, root = ".", options = {}) {
  let h;
  try {
    h = fileHash2(filePath);
  } catch {
    return null;
  }
  const entry = join12(cacheDir(root, options), `${h}.json`);
  if (!existsSync11(entry)) return null;
  try {
    return JSON.parse(readFileSync11(entry, "utf-8"));
  } catch {
    return null;
  }
}
function saveCached(filePath, result, root = ".", options = {}) {
  ...
  const h = fileHash2(filePath);
  const entry = join12(cacheDir(root, options), `${h}.json`);
  const tmp = entry + ".tmp";
  try {
    writeFileSync10(tmp, JSON.stringify(result));
...
function checkSemanticCache(files, root = ".", options = {}) {
  const cachedNodes = [];
  const cachedEdges = [];
  const cachedHyperedges = [];
  const uncached = [];
...
function saveSemanticCache(nodes, edges, hyperedges = null, root = ".", options = {}) {
  ...
  for (const [fpath, result] of byFile) {
    const p = resolve12(root, fpath);
    try {
      if (statSync5(p).isFile()) {
        saveCached(p, result, root, options);
        saved++;
      }
    } catch {
      continue;
...
import { readFileSync as readFileSync12, readdirSync as readdirSync4, lstatSync as lstatSync3, realpathSync, existsSync as existsSync12 } from "fs";
import { resolve as resolve13, basename as basename6, extname as extname7, dirname as dirname8, join as join13, relative as relative5, sep as sep3 } from "path";
...
function resolveGrammarWasm(langName) {
  ...
  const nmDir = join13(process.cwd(), "node_modules");
  for (const candidate of candidates) {
    const p = join13(nmDir, candidate);
...
function inferCommonRoot(paths) {
  try {
    ...
    if (paths.length === 1) {
      return resolve13(dirname8(paths[0]));
    }
    const parts = paths.map((p) => resolve13(p).split(sep3));
    ...
    return commonLen > 0 ? resolve13(parts[0].slice(0, commonLen).join(sep3)) : resolve13(".");
  } catch {
    return resolve13(".");
  }
}
function projectRelativeFilePath(filePath, root) {
  ...
  const rel3 = relative5(root, resolvedPath);
  if (!rel3 || rel3.startsWith("..")) {
    return toPortablePath(resolvedPath);
  }
  return toPortablePath(rel3);
}
function remapFileNodeIds(nodes, edges, paths, root) {
  ...
  for (const filePath of paths) {
    ...
    byPath.set(resolvedPath, {
      legacyId: _makeId(basename6(resolvedPath, extname7(resolvedPath))),
      absoluteId,
      portableId,
      label: basename6(resolvedPath)
    });
    absoluteToPortable.set(absoluteId, portableId);
  }
  for (const node of nodes) {
    const info = byPath.get(resolve13(node.source_file ?? ""));
  ...
  for (const edge of edges) {
    const sourceInfo = byPath.get(resolve13(edge.source_file ?? ""));
...
function _importJs(node, source, fileNid, _stem, edges, strPath) {
  for (const child of node.children) {
    if (child.type === "string") {
      ...
      let tgtNid = null;
      if (raw.startsWith(".")) {
        let resolvedImport = resolve13(dirname8(strPath), raw);
        const ext = extname7(resolvedImport).toLowerCase();
        if (ext === ".js") {
...
async function _extractGeneric(filePath, config) {
  ...
  const stem2 = basename6(filePath, extname7(filePath));
  ...
  addNode(fileNid, basename6(filePath), 1);
  ...
  function walk(node, scopeNid) {
    ...
    if (t === "struct_definition") {
      ...
      if (typeHead) {
        ...
        if (binExpr) {
          const identifiers2 = binExpr.children.filter((c) => c.type === "identifier");
          if (identifiers2.length > 0) {
            const structName = _readText(identifiers2[0], source);
            const structNid = _makeId(stem2, structName);
            const line = node.startPosition.row + 1;
            addNode(structNid, structName, line);
            addEdge(scopeNid, structNid, "defines", line);
            if (identifiers2.length >= 2) {
              const superName = _readText(identifiers2[identifiers2.length - 1], source);
    ...
    if (t === "using_statement" || t === "import_statement") {
      ...
      for (const child of node.children) {
        if (child.type === "identifier") {
          ...
          const identifiers2 = child.children.filter((c) => c.type === "identifier");
          if (identifiers2.length > 0) {
            const pkgName = _readText(identifiers2[0], source);
...
async function extractGo(filePath) {
  ...
  const root = tree.rootNode;
  const stem2 = basename6(filePath, extname7(filePath));
  const pkgScope = dirname8(filePath).split(sep3).pop() || stem2;
  const strPath = filePath;
  const nodes = [];
  const edges = [];
  const seenIds = /* @__PURE__ */ new Set();
...
async function extractObjc(filePath) {
  ...
  function walk(node, parentNid = null) {
    ...
    if (t === "class_interface") {
      const identifiers2 = node.children.filter((c) => c.type === "identifier");
      if (identifiers2.length === 0) {
        for (const child of node.children) walk(child, parentNid);
        return;
      }
      const name = _read(identifiers2[0]);
...
async function _resolveCrossFileImports(perFile, paths) {
  ...
  for (const fileResult of perFile) {
    for (const node of fileResult.nodes ?? []) {
      ...
      if (!src) continue;
      const fileStem = basename6(src, extname7(src));
  ...
  for (const p of paths) {
    stemToPath.set(basename6(p, extname7(p)), p);
  }
  for (let idx = 0; idx < perFile.length; idx++) {
    ...
    const fileStem = basename6(filePath, extname7(filePath));
    const strPath = filePath;
...
async function extractWithDiagnostics(paths) {
  const normalizedPaths = paths.map((filePath) => resolve13(filePath));
  const perFile = [];
  const diagnostics = [];
...
`);
    ...
    const ext = extname7(filePath);
    const extractor = basename6(filePath).endsWith(".blade.php") ? extractRegexBackedCode : _DISPATCH[ext];
...
`);
  ...
  const pyPaths = normalizedPaths.filter((p) => extname7(p) === ".py");
  if (pyPaths.length > 0) {
    const pyResults = perFile.filter((_r, i) => extname7(normalizedPaths[i]) === ".py");
    try {
...
function collectFiles(target, options) {
  const followSymlinks = options?.followSymlinks ?? false;
  const resolved = resolve13(target);
  try {
  ...
  function hasHiddenPartInsideRoot(path) {
    const rel3 = relative5(resolved, path);
    if (!rel3 || rel3.startsWith("..")) return false;
    return rel3.split(sep3).some((part) => part.startsWith("."));
  }
      ...
      if (stat.isDirectory() || stat.isSymbolicLink() && followSymlinks) {
        if (stat.isSymbolicLink()) {
          try {
            ...
            if (parentReal === real || parentReal.startsWith(real + sep3)) continue;
          } catch {
            continue;
          }
        }
...
// src/input-scope.ts
init_paths();
var DEFAULT_RECOMMENDATION = "Use --scope all or graphify.yaml inputs.corpus for a knowledge-base folder.";
...
var VALID_PDF_OCR_MODES = /* @__PURE__ */ new Set(["off", "auto", "always", "dry-run"]);
var VALID_CITATION_MINIMUMS = /* @__PURE__ */ new Set(["file", "page", "section", "paragraph"]);
var VALID_LLM_EXECUTION_MODES = /* @__PURE__ */ new Set(["assistant", "batch", "mesh", "off"]);
var VALID_IMAGE_ARTIFACT_SOURCES = /* @__PURE__ */ new Set(["ocr_crops", "images", "all"]);
var VALID_INPUT_SCOPE_MODES = /* @__PURE__ */ new Set(["auto", "committed", "tracked", "all"]);
...
function parseInputScopeMode(value, fallback) {
  const normalized = String(value ?? fallback).trim().toLowerCase();
...
// src/llm-execution.ts
import { mkdirSync, writeFileSync } from "fs";
import { dirname as dirname2, join as join4 } from "path";
function safeName(value) {
  return value.replace(/[^a-zA-Z0-9._-]+/gu, "-").replace(/^-+|-+$/gu, "") || "schema";
}
function writeInstruction(path, lines) {
  mkdirSync(dirname2(path), { recursive: true });
  writeFileSync(path, lines.join("\n") + "\n", "utf-8");
}
function preflightLlmExecution(policy, capability) {
  if (policy.mode === "off") {
...
function createAssistantTextJsonClient(options) {
  return {
    mode: "assistant",
    provider: "assistant",
    async generateJson(input) {
      const instructionPath = join4(options.instructionDir, `text-json-${safeName(input.schema)}.md`);
...
function createAssistantVisionJsonClient(options) {
  return {
    mode: "assistant",
    provider: "assistant",
    async analyzeImage(input) {
      const instructionPath = join4(options.instructionDir, `vision-json-${safeName(input.schema)}.md`);
      writeInstruction(instructionPath, [
...
// src/image-dataprep.ts
import { createHash } from "crypto";
import { existsSync as existsSync4, mkdirSync as mkdirSync2, readFileSync as readFileSync2, statSync as statSync2, writeFileSync as writeFileSync2 } from "fs";
import { basename as basename2, dirname as dirname3, extname as extname2, join as join5, resolve as resolve5 } from "path";
...
function fullPageScreenshot(path) {
  return /(^|[/\\])full[-_]page[-_]screenshots([/\\]|$)/iu.test(path);
}
function sourcePage(path) {
  const match = basename2(path).match(/(?:page|p)[-_]?(\d+)/iu);
  return match ? Number.parseInt(match[1], 10) : null;
}
function artifactId(path, sourceKind) {
  return `${sourceKind}-${sha256(resolve5(path)).slice(0, 16)}`;
}
function pdfArtifactByImage(pdfArtifacts) {
  const map = /* @__PURE__ */ new Map();
  for (const artifact of pdfArtifacts) {
    for (const imagePath of artifact.imagePaths ?? []) {
      map.set(resolve5(imagePath), artifact);
    }
  }
  return map;
}
function existingImages(paths, includeFullPageScreenshots) {
  return paths.map((item) => resolve5(item)).filter((item) => includeFullPageScreenshots || !fullPageScreenshot(item)).filter((item) => {
    try {
      return existsSync4(item) && statSync2(item).isFile();
    } catch {
      return false;
    }
  }).sort();
}
function buildImageDataprepManifest(options) {
  ...
  for (const imagePath of images) {
    const pdfArtifact = byImage.get(resolve5(imagePath));
    const sourceKind = pdfArtifact ? "ocr_crop" : "direct_image";
    artifacts.push({
...
function writeAssistantInstructions(path, manifest, config) {
...
function runImageDataprep(options) {
  ...
  const dir = join5(options.config.outputs.state_dir, "image-dataprep");
  const manifestPath = join5(dir, "manifest.json");
  const assistantInstructionsPath = policy.mode === "assistant" ? join5(dir, "assistant-instructions.md") : null;
...
// src/image-caption-schema.ts
var VALID_DENSITY = /* @__PURE__ */ new Set(["low", "medium", "high"]);
var VALID_ROUTING_SIGNAL = /* @__PURE__ */ new Set(["skip", "primary", "deep"]);
...
function validateImageCaption(value) {
  const issues = [];
...
function validateImageRouting(value) {
  const issues = [];
...
// src/image-routing-calibration.ts
...
function parseFile(path) {
  ...
  return path.endsWith(".json") ? JSON.parse(raw) : parseYaml2(raw);
}
...
function imageRoutingSampleFromCaption(artifactId2, caption) {
  const record = asRecord2(caption);
  const visualType = typeof record.visual_content_type === "string" && record.visual_content_type.trim() ? record.visual_content_type : "unknown";
  return {
...
function normalizeBucket(value) {
  ...
  const when = asRecord2(record.when);
  return {
...
function loadImageRoutingLabels(path) {
  const record = asRecord2(parseFile(path));
  ...
  return {
    ...
    labels: Array.isArray(record.labels) ? record.labels.map((item) => {
      const label = asRecord2(item);
      return {
        artifact_id: String(label.artifact_id ?? ""),
        label: String(label.label ?? ""),
        rationale: typeof label.rationale === "string" ? label.rationale : void 0
      };
    }) : []
  };
}
function loadImageRoutingRules(path) {
  const record = asRecord2(parseFile(path));
  ...
  const routes = asRecord2(record.routes);
  return {
    schema: "graphify_image_routing_rules_v1",
    decision: String(record.decision ?? "pending_labels"),
    routes: {
      skip: routes.skip === void 0 ? void 0 : normalizeBucket(routes.skip),
      primary: routes.primary === void 0 ? void 0 : normalizeBucket(routes.primary),
      deep: routes.deep === void 0 ? void 0 : normalizeBucket(routes.deep)
    }
  };
}
function assertAcceptedImageRoutingRules(rules) {
...
function writeImageRoutingCalibrationSamples(options) {
  const maxSamples = Math.max(0, Math.floor(options.maxSamples ?? options.manifest.artifacts.length));
  const allSamples = options.manifest.artifacts.map((artifact) => {
    const caption = parseFile(join6(options.captionsDir, `${artifact.id}.caption.json`));
    return imageRoutingSampleFromCaption(artifact.id, caption);
  }).sort((left, right) => left.visual_content_type.localeCompare(right.visual_content_type) || left.artifact_id.localeCompare(right.artifact_id));
  ...
  const runDir = join6(options.outputDir, options.runId);
  const samplesPath = join6(runDir, "samples.json");
  const payload = {
  ...
  writeFileSync3(samplesPath, JSON.stringify(payload, null, 2) + "\n", "utf-8");
  return { runDir, samplesPath, sampleCount: samples.length };
}
function bucketMatches(bucket, sample) {
  ...
  if (minRelationships !== void 0 && sample.relationship_count < minRelationships) return false;
  const minEntities = bucket.when?.min_entity_candidates;
  if (minEntities !== void 0 && sample.entity_count < minEntities) return false;
  return types.length > 0 || minRelationships !== void 0 || minEntities !== void 0;
}
function routeImageWithRules(rules, sample) {
  for (const route of ["deep", "primary", "skip"]) {
    ...
    if (bucketMatches(bucket, sample)) {
      ...
      if (bucket?.when?.min_relationship_candidates !== void 0) {
        reasons.push(`relationship_count>=${bucket.when.min_relationship_candidates}`);
      }
      if (bucket?.when?.min_entity_candidates !== void 0) {
...
function calibrateImageRouting(input) {
  ...
  const decision = missingLabels > 0 || ambiguousLabels > 0 ? "pending_labels" : falsePrimary > 0 ? "revise_matrix" : input.samples.length === 0 ? "pending_labels" : "accept_matrix";
  return {
    decision,
    metrics: {
...
function artifactHasDeepRoute(artifactId2, options) {
  ...
  const caption = readCaption(join7(options.captionsDir, `${artifactId2}.caption.json`));
  const errors = validateImageCaption(caption);
  if (errors.length > 0) {
    throw new Error(`Invalid caption sidecar for ${artifactId2}:
${errors.map((item) => `  - ${item}`).join("\n")}`);
  }
  return routeImageWithRules(options.rules, imageRoutingSampleFromCaption(artifactId2, caption)).route === "deep";
}
function exportImageDataprepBatchRequests(options) {
  mkdirSync4(dirname4(options.outputPath), { recursive: true });
  const artifacts = options.pass === "deep" ? options.manifest.artifacts.filter((artifact) => artifactHasDeepRoute(artifact.id, options)) : options.manifest.artifacts;
  ...
  writeFileSync4(options.outputPath, content, "utf-8");
  return { outputPath: options.outputPath, requestCount: artifacts.length };
}
function existingValidSidecarErrors(captionPath, routingPath) {
  const errors = [];
  if (existsSync5(captionPath) && validateImageCaption(readCaption(captionPath)).length === 0) {
    errors.push(`caption sidecar already exists: ${captionPath}`);
  }
  if (existsSync5(routingPath) && validateImageRouting(readCaption(routingPath)).length === 0) {
    errors.push(`routing sidecar already exists: ${routingPath}`);
  }
  return errors;
}
function importImageDataprepBatchResults(options) {
  const captionsDir = join7(options.outputDir, "captions");
  const routingDir = join7(options.outputDir, "routing");
  const failures = [];
  let importedCount = 0;
  for (const item of readJsonl(options.inputPath)) {
    const record = asRecord3(item);
    const artifactId2 = String(record.artifact_id ?? record.id ?? "");
    const caption = record.caption;
    const routing = record.routing;
    const captionPath = join7(captionsDir, `${artifactId2}.caption.json`);
    const routingPath = join7(routingDir, `${artifactId2}.routing.json`);
    const errors = [
      ...validateImageCaption(caption),
      ...validateImageRouting(routing)
    ];
    if (!artifactId2) errors.push("artifact_id is required");
    if (!options.force) {
      errors.push(...existingValidSidecarErrors(captionPath, routingPath));
    }
    if (errors.length > 0) {
      failures.push({ artifact_id: artifactId2 || "unknown", errors });
      continue;
    }
    mkdirSync4(captionsDir, { recursive: true });
    mkdirSync4(routingDir, { recursive: true });
    writeFileSync4(captionPath, JSON.stringify(caption, null, 2) + "\n", "utf-8");
    writeFileSync4(routingPath, JSON.stringify(routing, null, 2) + "\n", "utf-8");
    importedCount++;
  }
...
function compileNodes(extraction, profile, config) {
  ...
  const nodes = extraction.nodes.filter((node) => {
    ...
    const type = ontologyNodeType(node);
    const aliases2 = stringArray2(node.aliases);
    ...
    return {
      ...
      registry_refs: stringArray2(node.registry_refs),
...
function writeWiki(outputDir, nodes, relations, config) {
  ...
  const wikiDir = join8(outputDir, "wiki");
  const entityDir = join8(wikiDir, "entities");
  ...
  for (const node of nodes.filter((item) => pageTypes.has(item.type))) {
    ...
    writeFileSync5(join8(entityDir, `${safeFilename(node.id)}.md`), lines.join("\n") + "\n", "utf-8");
    count++;
  }
  writeJson(join8(wikiDir, "index.json"), { entries: nodes.map((node) => ({ id: node.id, label: node.label, type: node.type })) });
  writeFileSync5(join8(wikiDir, "index.md"), nodes.map((node) => `- [${node.label}](entities/${safeFilename(node.id)}.md)`).join("\n") + "\n", "utf-8");
  return count;
}
function compileOntologyOutputs(options) {
  ...
  writeJson(join8(options.outputDir, "manifest.json"), manifest);
  writeJson(join8(options.outputDir, "nodes.json"), nodes);
  writeJson(join8(options.outputDir, "aliases.json"), nodes.flatMap(
  ...
  writeJson(join8(options.outputDir, "relations.json"), relations);
  writeJson(join8(options.outputDir, "sources.json"), nodes.flatMap((node) => node.source_refs.map((ref) => ({ id: ref, source_file: ref }))));
  writeJson(join8(options.outputDir, "occurrences.json"), []);
  writeJson(join8(options.outputDir, "validation.json"), { issues: validationIssues });
  writeJson(join8(options.outputDir, "index.json"), {
...
import { extname as extname3, resolve as resolve6 } from "path";
import { parse as parseYaml3 } from "yaml";
var DEFAULT_STATUSES = ["candidate", "attached", "needs_review", "validated", "rejected", "superseded"];
var VALID_CITATION_MINIMUMS2 = /* @__PURE__ */ new Set(["file", "page", "section", "paragraph"]);
...
function normalizeStringMap(value) {
  ...
  return Object.fromEntries(
    Object.entries(record).filter(([key, item]) => key.trim().length > 0 && asRecord4(item))
  );
}
...
function normalizeOutputs(outputs) {
  const outputRecord = asRecord4(outputs);
  const ontology = asRecord4(outputRecord.ontology);
  const wiki = asRecord4(ontology.wiki);
  return {
    ontology: {
...
function parseOntologyProfile(raw, sourcePath) {
  if (typeof raw !== "string") return asRecord4(raw);
  const trimmed = raw.trim();
  const parsed = extname3(sourcePath ?? "").toLowerCase() === ".json" || trimmed.startsWith("{") ? JSON.parse(trimmed || "{}") : parseYaml3(trimmed || "{}");
  return asRecord4(parsed);
}
...
${errors.map((item) => `  - ${item}`).join("\n")}`);
  ...
  const normalized = {
    ...
    ...sourcePath ? { sourcePath: resolve6(sourcePath) } : profile.sourcePath ? { sourcePath: profile.sourcePath } : {},
...
function loadOntologyProfile(profilePath, options = {}) {
  const resolved = resolve6(profilePath);
...
import { extname as extname4, resolve as resolve7 } from "path";
...
function normalizeRegistryRecord(registryId, registrySpec, rawRecord, sourceFile) {
  ...
  return {
    ...
    sourceFile: resolve7(sourceFile),
    raw: { ...rawRecord }
  };
}
function loadProfileRegistry(registryId, registrySpec) {
  ...
  const sourceFile = resolve7(registrySpec.bound_source_path);
...
function validateCitations(entity, profile, issues, where) {
  ...
  if (profile.citation_policy.require_source_file) {
    if (entityCitations.length === 0 || !entityCitations.some((citation) => stringValue2(citation.source_file))) {
      addIssue(issues, {
        severity: "error",
...
function validateStatus(entity, profile, issues, where) {
  const status = stringValue2(entity.status);
  if (!status) return;
...
function isRegistrySeed(node) {
  return Boolean(stringValue2(node.registry_id) && stringValue2(node.registry_record_id));
}
function validateNode(node, profile, issues, index) {
  ...
  if (profileNodeType.registry && (!stringValue2(node.registry_id) || !stringValue2(node.registry_record_id))) {
    addIssue(issues, {
      severity: "warning",
...
function isProfileEdge(edge, profile, sourceNode, targetNode) {
  const relation = stringValue2(edge.relation);
  return Boolean(
    profile.relation_types[relation] || stringValue2(sourceNode?.node_type) || stringValue2(targetNode?.node_type) || stringValue2(edge.status) || citations(edge).length > 0
  );
}
function validateEdge(edge, profile, nodesById, issues, index) {
  ...
  const sourceType = stringValue2(sourceNode?.node_type);
  ...
  const targetType = stringValue2(targetNode?.node_type);
...
function relationTypeSection(profile) {
  const lines = Object.entries(profile.relation_types).map(
    ([relation, spec]) => `- ${relation}: ${spec.source_types.join(" | ")} -> ${spec.target_types.join(" | ")}`
  );
  return ["## Allowed relation types", ...lines].join("\n");
}
function registrySection(state, options) {
...
function citationSection(profile) {
  return [
    "## Citation policy",
...
function hardeningSection(profile) {
  return [
...
function inputHintsSection(state) {
...
function chunkGuidance(fileType) {
  if (fileType === "image") {
    return [
      "## Image guidance",
...
function buildProfileExtractionPrompt(state, options = {}) {
  return [
...
function buildProfileChunkPrompt(state, chunk, options = {}) {
  return [
...
function buildProfileValidationPrompt(state, extraction, options = {}) {
  return [
...
function rel2(config, path) {
  if (!config) return path;
  const value = relative3(config.configDir, path).replace(/\\/g, "/");
  return value || ".";
}
...
function projectConfigSection(context) {
  ...
  return [
    ...
    `- config: ${rel2(config, config.sourcePath)}`,
    `- corpus: ${config.inputs.corpus.map((path) => rel2(config, path)).join(", ") || "none"}`,
    `- generated: ${config.inputs.generated.map((path) => rel2(config, path)).join(", ") || "none"}`,
    `- excluded: ${config.inputs.exclude.map((path) => rel2(config, path)).join(", ") || "none"}`,
    `- state_dir: ${rel2(config, config.outputs.state_dir)}`
  ];
}
function registryCoverageSection(context) {
  ...
  for (const [registryId, count] of Object.entries(context.profileState.registry_counts)) {
    ...
    const attachedIds = new Set(
      nodes.filter((node) => stringValue3(node.registry_id) === registryId).map((node) => stringValue3(node.registry_record_id)).filter(Boolean)
    );
...
function unattachedEntitiesSection(context) {
  ...
  const lines = nodes.filter((node) => {
    const nodeType = stringValue3(node.node_type);
    return Boolean(nodeType && context.profile.node_types[nodeType]?.registry && !stringValue3(node.registry_record_id));
  }).slice(0, 20).map((node) => `- ${stringValue3(node.label) || stringValue3(node.id)} (${stringValue3(node.node_type)})`);
  return [
...
function invalidRelationsSection(context) {
  ...
  const ambiguousLines = graphLinks(context).filter((link) => stringValue3(link.confidence) === "AMBIGUOUS").map((link) => `- AMBIGUOUS ${stringValue3(link.relation)}: ${stringValue3(link.source)} -> ${stringValue3(link.target)}`);
  return [
...
function highDegreeSection(context) {
  const nodes = graphNodes(context);
  const labels = new Map(nodes.map((node) => [stringValue3(node.id), stringValue3(node.label) || stringValue3(node.id)]));
  const degree = /* @__PURE__ */ new Map();
  for (const link of graphLinks(context)) {
    const source = stringValue3(link.source);
    const target = stringValue3(link.target);
...
function lowEvidenceSection(context) {
  ...
  for (const link of graphLinks(context)) {
    const relation = stringValue3(link.relation) || "unknown";
    const entry = counts.get(relation) ?? { inferred: 0, ambiguous: 0 };
    if (stringValue3(link.confidence) === "INFERRED") entry.inferred += 1;
    if (stringValue3(link.confidence) === "AMBIGUOUS") entry.ambiguous += 1;
...
function humanReviewSection(context) {
  const lines = graphNodes(context).filter((node) => ["candidate", "needs_review"].includes(stringValue3(node.status))).slice(0, 20).map((node) => `- ${stringValue3(node.label) || stringValue3(node.id)} (${stringValue3(node.status)})`);
...
import { existsSync as existsSync7, mkdirSync as mkdirSync6, statSync as statSync3 } from "fs";
import { homedir } from "os";
import { basename as basename3, join as join9, resolve as resolve9 } from "path";
...
function repoNameFromUrl(url) {
  ...
  try {
    if (url.startsWith("file://")) {
      return basename3(fileURLToPath(url)).replace(/\.git$/i, "") || "repo";
    }
    if (/^[a-z]+:\/\//i.test(url)) {
      const parsed = new URL(url);
      return basename3(parsed.pathname).replace(/\.git$/i, "") || "repo";
    }
  } catch {
  }
  return basename3(url).replace(/\.git$/i, "") || "repo";
}
function defaultCloneDestination(url, cacheRoot = join9(homedir(), ".graphify", "repos")) {
  const github = maybeGithubRepo(url);
  if (github) {
    return resolve9(cacheRoot, github.owner, github.repo);
  }
  return resolve9(cacheRoot, "external", repoNameFromUrl(url));
}
function cloneRepo(options) {
  ...
  const destination = resolve9(options.outDir ?? defaultCloneDestination(options.url, options.cacheRoot));
  if (existsSync7(destination)) {
    if (!statSync3(destination).isDirectory()) {
      throw new Error(`clone destination exists and is not a directory: ${destination}`);
    }
    if (!existsSync7(join9(destination, ".git"))) {
  ...
  mkdirSync6(join9(destination, ".."), { recursive: true });
...
// src/merge-graphs.ts
init_graph();
import { existsSync as existsSync8, mkdirSync as mkdirSync7, readFileSync as readFileSync8, writeFileSync as writeFileSync6 } from "fs";
import { basename as basename4, dirname as dirname6, resolve as resolve10 } from "path";
function repoTagFromGraphPath(graphPath) {
  const parentName = basename4(dirname6(graphPath));
  if (parentName === ".graphify" || parentName === "graphify-out") {
    return basename4(dirname6(dirname6(graphPath))) || "unknown";
  }
...
function mergeGraphsFromFiles(options) {
  ...
  const graphs = options.inputs.map((inputPath) => {
    const resolved = resolve10(inputPath);
    if (!existsSync8(resolved)) {
      throw new Error(`graph file not found: ${resolved}`);
    }
    const raw = JSON.parse(readFileSync8(resolved, "utf-8"));
  ...
  writeFileSync6(outPath, JSON.stringify(serializeGraph(merged), null, 2), "utf-8");
  return {
    graph: merged,
    out: outPath,
...
function uniquePageRefs(pages) {
  const used = /* @__PURE__ */ new Set();
  const refs = /* @__PURE__ */ new Map();
  for (const page of pages) {
    const base = safeFilename2(page.title) || "page";
    let stem2 = base;
    let suffix = 2;
    while (used.has(stem2)) {
      stem2 = `${base}_${suffix}`;
      suffix += 1;
    }
    used.add(stem2);
    refs.set(page.key, {
      ...
      title: page.title,
      filename: `${stem2}.md`,
      link: stem2 === base ? `[[${page.title}]]` : `[[${stem2}|${page.title}]]`
    });
  }
  return refs;
}
function normalizeFlows2(input) {
  if (!input) return [];
  return Array.isArray(input) ? input : input.flows;
}
function compareFlowCriticality2(a, b) {
  return b.criticality - a.criticality || a.name.localeCompare(b.name);
}
function flowsThroughNodes(flows, nodes) {
  const nodeSet = new Set(nodes);
  return flows.filter((flow) => flow.path.some((nodeId) => nodeSet.has(nodeId))).sort(compareFlowCriticality2);
}
function crossCommunityLinks(G, nodes, ownCid) {
  const counts = /* @__PURE__ */ new Map();
...
function communityArticle(G, cid, nodes, label, labels, cohesion, communityLinks, flows, flowLinks) {
  ...
  const sources = [...new Set(nodes.map((n) => G.getNodeAttribute(n, "source_file") ?? "").filter(Boolean))].sort();
  const lines = [];
  lines.push(`# ${label}`, "");
  const metaParts = [`${nodes.length} nodes`];
  if (cohesion !== void 0) metaParts.push(`cohesion ${cohesion.toFixed(2)}`);
  ...
  if (communityFlows.length > 0) {
    ...
    for (const flow of communityFlows.slice(0, 12)) {
      ...
      lines.push(`- ${link} \u2014 criticality ${flow.criticality.toFixed(4)} \xB7 ${flow.nodeCount} nodes \xB7 ${flow.fileCount} files`);
    }
    lines.push("");
  }
  if (sources.length > 0) {
...
function godNodeArticle(G, nid, labels, communityLinks) {
  const d = G.getNodeAttributes(nid);
  const nodeLabel = d.label ?? nid;
  const src = d.source_file ?? "";
  ...
  if (communityName2) {
    lines.push(`**Community:** ${communityLinks.get(cid) ?? `[[${communityName2}]]`}`, "");
  }
  ...
  for (const neighbor of neighbors) {
    ...
    const rel3 = ed.relation ?? "related";
    const neighborLabel = G.getNodeAttribute(neighbor, "label") ?? neighbor;
    const conf = ed.confidence ?? "";
    const confStr = conf ? ` \`${conf}\`` : "";
    if (!byRelation.has(rel3)) byRelation.set(rel3, []);
    byRelation.get(rel3).push(`[[${neighborLabel}]]${confStr}`);
  }
  lines.push("## Connections by Relation", "");
  for (const [rel3, targets] of [...byRelation.entries()].sort()) {
    lines.push(`### ${rel3}`);
...
function flowArticle(G, flow, communityLinks) {
  const lines = [];
  lines.push(`# Flow ${flow.name}`, "");
  lines.push(
    `> Execution flow \xB7 criticality ${flow.criticality.toFixed(4)} \xB7 depth ${flow.depth} \xB7 ${flow.nodeCount} nodes \xB7 ${flow.fileCount} files`,
    ""
  );
  ...
  flow.path.forEach((nodeId, index) => {
    ...
    const label = attrs.label ?? flow.qualifiedPath[index] ?? nodeId;
    const file = attrs.source_file ?? "";
    const cid = attrs.community;
    const community = cid !== void 0 ? ` \xB7 ${communityLinks.get(cid) ?? `[[Community ${cid}]]`}` : "";
    const filePart = file ? ` \xB7 \`${file}\`` : "";
...
function indexMd(communities, labels, communityLinks, godNodesData, flowPages, totalNodes, totalEdges) {
  const lines = [
  ...
  if (flowPages.length > 0) {
    ...
    for (const { flow, page } of flowPages.sort((a, b) => compareFlowCriticality2(a.flow, b.flow))) {
      lines.push(`- ${page.link} \u2014 criticality ${flow.criticality.toFixed(4)} \xB7 ${flow.nodeCount} nodes`);
    }
    lines.push("");
  }
  lines.push(
...
function toWiki(G, communities, outputDir, options) {
  ...
  const flows = normalizeFlows2(options?.flows);
  ...
  for (const [cid, nodes] of communityMap) {
    ...
    writeFileSync9(join11(outputDir, page?.filename ?? `${safeFilename2(label)}.md`), article);
    count++;
  }
  for (const nodeData of godNodesData) {
    ...
    if (nid && G.hasNode(nid)) {
      ...
      writeFileSync9(join11(outputDir, page?.filename ?? `${safeFilename2(nodeData.label)}.md`), article);
      count++;
    }
  }
  for (const flow of flows) {
    const page = pageRefs.get(`flow:${flow.id}`);
    writeFileSync9(join11(outputDir, page?.filename ?? `${safeFilename2(`Flow ${flow.name}`)}.md`), flowArticle(G, flow, communityLinks));
    count++;
  }
  writeFileSync9(
    join11(outputDir, "index.md"),
    indexMd(
      communityMap,
      labels,
      communityLinks,
...
// src/lifecycle.ts
import { existsSync as existsSync13, mkdirSync as mkdirSync11, readFileSync as readFileSync13, unlinkSync as unlinkSync2, writeFileSync as writeFileSync11 } from "fs";
...
function lifecyclePaths(root = ".") {
  const paths = resolveGraphifyPaths({ root });
  return {
    worktree: join14(paths.stateDir, "worktree.json"),
    branch: join14(paths.stateDir, "branch.json")
  };
}
...
function refreshLifecycleMetadata(root = ".", options = {}) {
  const rootResolved = resolve14(root);
  const paths = resolveGraphifyPaths({ root: rootResolved });
  ...
  writeJson2(metadataPaths.worktree, worktree);
  writeJson2(metadataPaths.branch, branch);
  if (stale) {
    writeFileSync11(paths.needsUpdate, "1", "utf-8");
  } else if (existsSync13(paths.needsUpdate)) {
...
function planLifecyclePrune(root = ".") {
  ...
  if (metadata?.branch.stale && !existsSync13(paths.graph)) {
    candidates.push({
...
function loadGraph(graphPath) {
  const raw = JSON.parse(readFileSync14(graphPath, "utf-8"));
...
// src/ingest.ts
init_security();
import { existsSync as existsSync16, mkdirSync as mkdirSync13, writeFileSync as writeFileSync13 } from "fs";
import { resolve as pathResolve2, basename as basename8, extname as extname9 } from "path";
...
import { basename as basename7, extname as extname8, join as join15, resolve as resolve15 } from "path";
...
var FASTER_WHISPER_PACKAGE = "faster-whisper-ts";
var HUGGING_FACE_RESOLVE_BASE = "https://huggingface.co";
var REQUIRED_MODEL_FILES = ["config.json", "model.bin", "tokenizer.json", "vocabulary.txt"];
...
var fasterWhisperModulePromise = null;
...
function defaultWhisperCacheDir() {
  if (process.env.GRAPHIFY_WHISPER_CACHE_DIR) {
    return resolve15(process.env.GRAPHIFY_WHISPER_CACHE_DIR);
  }
  if (platform() === "win32") {
    return join15(
      process.env.LOCALAPPDATA ?? join15(homedir2(), "AppData", "Local"),
      "graphify",
      "faster-whisper"
    );
  }
  return join15(process.env.XDG_CACHE_HOME ?? join15(homedir2(), ".cache"), "graphify", "faster-whisper");
}
...
function missingModelFiles(modelDir) {
  return REQUIRED_MODEL_FILES.filter((fileName) => !existsSync15(join15(modelDir, fileName)));
}
...
async function ensureWhisperArtifacts(modelName) {
  ...
  if (explicitModelDir) {
    const modelDir2 = resolve15(explicitModelDir);
  ...
  const modelDir = join15(cacheRoot, sanitizeCacheSegment(repoId) + "-" + sanitizeCacheSegment(revision));
  ...
  const tempDir = mkdtempSync(join15(tmpdir(), "graphify-whisper-model-"));
  try {
    ...
    for (const fileName of REQUIRED_MODEL_FILES) {
      await downloadFile(modelDownloadUrl(repoId, revision, fileName), join15(tempDir, fileName));
    }
    validateWhisperModelDir(tempDir, resolved);
    rmSync(modelDir, { recursive: true, force: true });
    mkdirSync12(modelDir, { recursive: true });
    for (const fileName of REQUIRED_MODEL_FILES) {
      writeFileSync12(join15(modelDir, fileName), await readFile(join15(tempDir, fileName)));
    }
    return { requestedModel: requested, resolvedModel: resolved, modelDir, repoId, revision };
  } catch (error) {
...
function downloadAudio(url, outputDir) {
  ...
  const urlHash = createHash6("sha1").update(url).digest("hex").slice(0, 12);
  for (const ext of CACHED_AUDIO_EXTENSIONS) {
    const candidate = join15(outputDir, "yt_" + urlHash + ext);
    if (existsSync15(candidate)) {
      console.log("  cached audio: " + basename7(candidate));
      return candidate;
    }
  }
  const outTemplate = join15(outputDir, "yt_" + urlHash + ".%(ext)s");
  try {
  ...
  for (const entry of readdirSync5(outputDir)) {
    if (entry.startsWith("yt_" + urlHash + ".")) {
      return join15(outputDir, entry);
    }
  }
...
async function transcribe(videoPath, outputDir = defaultTranscriptsDir(), initialPrompt, force = false) {
  const outDir = resolve15(outputDir);
  mkdirSync12(outDir, { recursive: true });
  const audioPath = isUrl(videoPath) ? downloadAudio(videoPath, join15(outDir, "downloads")) : resolve15(videoPath);
  const transcriptPath = join15(outDir, basename7(audioPath, extname8(audioPath)) + ".txt");
  if (existsSync15(transcriptPath) && !force) {
  ...
  try {
    console.log("  transcribing " + basename7(audioPath) + " (model=" + requestedModel + ") ...");
    ...
    writeFileSync12(transcriptPath, transcript, "utf-8");
...
function safeFilename3(url, suffix) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
...
Source: ${url}
`;
  const filename = safeFilename3(url, ".md");
...
async function downloadBinary(url, suffix, targetDir) {
  const filename = safeFilename3(url, suffix);
  const outPath = pathResolve2(targetDir, filename);
  const data = await safeFetch(url);
  writeFileSync13(outPath, data);
  return outPath;
}
...
async function ingest(url, targetDir, authorOrOptions = null, contributor = null) {
  ...
  if (urlType === "pdf") {
    ...
    console.log(`Downloaded PDF: ${basename8(out)}`);
    return out;
  }
  if (urlType === "image") {
    let parsed;
    ...
    const suffix = extname9(parsed.pathname) || ".jpg";
    const out = await downloadBinary(url, suffix, targetDir);
    console.log(`Downloaded image: ${basename8(out)}`);
    return out;
  }
  if (urlType === "youtube") {
    const out = downloadAudio(url, targetDir);
    console.log(`Downloaded audio: ${basename8(out)}`);
    return out;
  }
  if (urlType === "tweet") {
  ...
  let counter = 1;
  while (existsSync16(outPath)) {
    const stem2 = filename.replace(/\.md$/, "");
    outPath = pathResolve2(targetDir, `${stem2}_${counter}.md`);
    counter++;
  }
  writeFileSync13(outPath, content, "utf-8");
  console.log(`Saved ${urlType}: ${basename8(outPath)}`);
  return outPath;
}
...
var isDirectExecution = typeof process !== "undefined" && typeof process.argv[1] === "string" && /^ingest\.(?:js|mjs|cjs|ts)$/.test(basename8(process.argv[1]));
...
import { basename as basename9, extname as extname10, resolve as resolve16 } from "path";
var DEFAULT_MIN_WORDS_PER_PAGE = 25;
var DEFAULT_MIN_TOTAL_WORDS = 40;
var IMAGE_MARKER_PATTERN = /\/(?:Image|XObject)\b/g;
...
function extractWithPdftotext(filePath) {
  const textResult = childProcess2.spawnSync("pdftotext", [filePath, "-"], {
    encoding: "utf-8",
  ...
  let pageCount = 1;
  const infoResult = childProcess2.spawnSync("pdfinfo", [filePath], {
    encoding: "utf-8",
    maxBuffer: 1024 * 1024
  });
...
async function extractPdfTextLayer(filePath, buffer = readFileSync15(filePath), preferredProvider) {
...
async function preflightPdf(filePath, mode = parsePdfOcrMode2(), options = {}) {
  ...
  const buffer = readFileSync15(resolved);
  const hash = sha2563(buffer);
...
function pdfOcrSidecarStem(filePath, sha) {
  return basename9(filePath, extname10(filePath)).replace(/[^a-zA-Z0-9_.-]+/g, "_") + "_" + sha.slice(0, 12);
}

// src/pdf-ocr.ts
import { existsSync as existsSync17, mkdirSync as mkdirSync14, readdirSync as readdirSync6, readFileSync as readFileSync16, statSync as statSync6, writeFileSync as writeFileSync14 } from "fs";
import { join as join16, resolve as resolve17 } from "path";
var MISTRAL_OCR_PACKAGE = "mistral-ocr";
var DEFAULT_OCR_MODEL = "mistral-ocr-latest";
var PDF_IMAGE_EXTENSIONS = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".webp", ".gif"]);
var mistralOcrModulePromise = null;
...
function listImageArtifacts(imageOutputDir) {
  if (!existsSync17(imageOutputDir)) return [];
  return readdirSync6(imageOutputDir).map((entry) => join16(imageOutputDir, entry)).filter((entryPath) => {
    try {
      const extension = entryPath.slice(entryPath.lastIndexOf(".")).toLowerCase();
      return PDF_IMAGE_EXTENSIONS.has(extension) && statSync6(entryPath).isFile();
    } catch {
      return false;
    }
  }).sort();
...
function writeMetadata(path, artifact) {
  writeFileSync14(path, JSON.stringify({
...
async function preparePdf(filePath, outputDir, options) {
  ...
  const markdownPath = join16(outputDir, stem2 + ".md");
  const imageOutputDir = join16(outputDir, stem2 + "_images");
  const ocrMetadataPath = metadataPath(markdownPath);
  if (existsSync17(markdownPath)) {
    const provider = preflight.shouldOcr ? "mistral-ocr" : preflight.textLayerProvider === "pdftotext" ? "pdftotext" : "pdf-parse";
    const artifact = {
      sourceFile: resolve17(filePath),
      markdownPath,
  ...
  if (!preflight.shouldOcr) {
    if (preflight.reason === "text_extractable") {
      ...
      if (countWords3(text) > 0 && textLayer.provider !== "none") {
        writeFileSync14(markdownPath, textMarkdown(resolve17(filePath), text, textLayer.provider), "utf-8");
        const artifact = {
          sourceFile: resolve17(filePath),
          markdownPath,
  ...
  try {
    ...
    await mistralOcr.convertPdf(resolve17(filePath), {
      apiKey,
    ...
    const originalMarkdown = existsSync17(markdownPath) ? readFileSync16(markdownPath, "utf-8") : "";
    if (originalMarkdown && !originalMarkdown.startsWith("---\n")) {
      writeFileSync14(
        markdownPath,
        [
          "---",
          "graphify_source_file: " + JSON.stringify(resolve17(filePath)),
...
async function augmentDetectionWithPdfPreflight(detection, options = {}) {
  ...
  const outputDir = resolve17(options.outputDir ?? join16(".graphify", "converted", "pdf"));
  ...
  for (const paperFile of paperFiles) {
    ...
    if (artifact.markdownPath && (artifact.status === "converted" || artifact.status === "cached")) {
      convertedSources.add(resolve17(paperFile));
  ...
  source.paper = paperFiles.filter((paperFile) => !convertedSources.has(resolve17(paperFile)));
...
function isCodePath(path) {
  return CODE_EXTENSIONS.has(extname11(path).toLowerCase());
}
...
init_security();
var MULTIMODAL_EXTENSIONS = /* @__PURE__ */ new Set([
  ".md",
  ".txt",
  ".rst",
  ".pdf",
...
function isMultimodalPath(path) {
  return MULTIMODAL_EXTENSIONS.has(extname12(path).toLowerCase());
}
...
function evaluateReviewAnalysis(G, cases, options = {}) {
  ...
  const results = cases.map((testCase) => {
    ...
    const analysisTokens = estimateTokens2(text);
...
function isTestPath2(path) {
  ...
  const normalized = normalizePath2(path).toLowerCase();
  return normalized.includes("/test/") || normalized.includes("/tests/") || normalized.includes("/__tests__/") || /(?:^|[._-])(test|spec)\.[^.]+$/.test(normalized);
}
...
function sortNodes(nodes) {
  return [...nodes].sort((a, b) => compareStrings4(a.name, b.name) || compareStrings4(a.qualifiedName, b.qualifiedName) || compareStrings4(a.filePath ?? "", b.filePath ?? ""));
}
function sortEdges(edges) {
  return [...edges].sort((a, b) => compareStrings4(a.sourceQualified, b.sourceQualified) || compareStrings4(a.targetQualified, b.targetQualified) || compareStrings4(a.kind, b.kind) || compareStrings4(a.id, b.id));
}
function createReviewGraphStore(G) {
  ...
  function getFilesMatching(pattern) {
    ...
    return [...files].sort(compareStrings4);
  }
  ...
  function getImpactRadius(changedFiles, options = {}) {
    ...
    const impactedFiles = [...new Set([...visited.values()].map((node) => node.filePath).filter((file) => !!file))].sort(compareStrings4);
    return {
  ...
  return {
    ...
    getAllFiles: () => [...new Set([...nodesById.values()].map((node) => node.filePath).filter((file) => !!file))].sort(compareStrings4),
    getEdgesBySource,
    getEdgesByTarget,
    ...
    getGraphStats: () => {
      ...
      return {
        ...
        nodesByKind,
        edgesByKind,
        languages: [...languages].sort(compareStrings4),
...
// src/flows.ts
import { mkdirSync as mkdirSync15, readFileSync as readFileSync17, writeFileSync as writeFileSync15 } from "fs";
import { dirname as dirname9, resolve as resolve18 } from "path";
var FLOW_ARTIFACT_VERSION = 1;
var DEFAULT_MAX_DEPTH = 15;
var FRAMEWORK_DECORATOR_PATTERNS = [
...
var TEST_FILE_RE = /([\\/]__tests__[\\/]|\.spec\.[jt]sx?$|\.test\.[jt]sx?$|[\\/]test_[^/\\]*\.py$)/u;
...
function stableFiles(nodes) {
  return [...new Set(nodes.map((node) => node.filePath).filter((file) => !!file))].sort(compareStrings5);
}
...
function buildFlowArtifact(store, options = {}) {
  ...
  return {
    ...
    warnings: [...new Set(result.warnings)].sort(compareStrings5),
    flows: result.flows
  };
}
function writeFlowArtifact(artifact, path) {
  const out = resolve18(path);
  mkdirSync15(dirname9(out), { recursive: true });
  writeFileSync15(out, `${JSON.stringify(artifact, null, 2)}
`, "utf-8");
}
function readFlowArtifact(path) {
  const artifact = JSON.parse(readFileSync17(resolve18(path), "utf-8"));
...
function listFlows(artifact, options = {}) {
  ...
  return [...artifact.flows].sort((a, b) => {
    switch (sortBy) {
      case "depth":
        return b.depth - a.depth || compareStrings5(a.name, b.name);
      case "node-count":
        return b.nodeCount - a.nodeCount || compareStrings5(a.name, b.name);
      case "file-count":
        return b.fileCount - a.fileCount || compareStrings5(a.name, b.name);
      case "name":
        return compareStrings5(a.name, b.name);
      case "criticality":
      default:
        return b.criticality - a.criticality || compareStrings5(a.name, b.name);
    }
  }).slice(0, limit);
}
...
function getAffectedFlows(artifact, changedFiles, store) {
  const normalizedFiles = [...new Set(changedFiles.map((file) => file.trim()).filter(Boolean))].sort(compareStrings5);
  ...
  const matched = [...matchedNodeIds].sort(compareStrings5);
  if (matched.length === 0) {
    return {
  ...
  const affectedFlows = artifact.flows.filter((flow) => flow.path.some((nodeId) => matchedSet.has(nodeId))).map((flow) => getFlowById(artifact, flow.id, store)).filter((flow) => !!flow && "steps" in flow).sort((a, b) => b.criticality - a.criticality || compareStrings5(a.name, b.name));
  return {
    changedFiles: normalizedFiles,
...
import { readFileSync as readFileSync18, realpathSync as realpathSync2, statSync as statSync7 } from "fs";
import { basename as basename10, relative as relative6, resolve as resolve19 } from "path";
...
function uniqueSorted3(values) {
  return [...new Set(values.map((value) => value.trim()).filter(Boolean).map(normalizePath3))].sort(compareStrings6);
}
...
function isSensitivePath(path) {
  ...
  const file = basename10(normalized);
  return file === ".env" || file === ".npmrc" || /\.(key|pem|p12|pfx|crt|cer)$/iu.test(file) || /(secret|credential|token)/iu.test(normalized);
}
function isInside(root, target) {
  const rel3 = relative6(root, target);
  return rel3 === "" || !rel3.startsWith("..") && !resolve19(rel3).startsWith("..");
}
...
function extractRelevantLines(lines, nodes, filePath) {
  const ranges = [];
  for (const node of nodes) {
    if (!sourceMatches2(node.filePath, filePath) || node.lineStart === null || node.lineEnd === null) continue;
...
function readSourceSnippet(changedFile, changedNodes, options) {
  ...
  try {
    const root = realpathSync2(resolve19(options.repoRoot));
    const fullPath = realpathSync2(resolve19(root, changedFile));
    if (!isInside(root, fullPath) || !statSync7(fullPath).isFile()) return "(could not read file)";
...
function buildReviewContext(store, changedFilesInput, options = {}) {
  const changedFiles = uniqueSorted3(changedFilesInput);
...
var SAFE_GIT_REF_RE = /^(?!-)(?!.*\.\.)[A-Za-z0-9_.~^/@{}-]+$/u;
var SECURITY_KEYWORDS2 = [
  "auth",
  "login",
  "password",
  "token",
...
function uniqueSorted4(values) {
  return [...new Set(values.map((value) => value.trim()).filter(Boolean))].sort(compareStrings7);
}
...
function sortNodesByLocation(nodes) {
  return [...nodes].sort((a, b) => compareStrings7(a.filePath ?? "", b.filePath ?? "") || (a.lineStart ?? Number.MAX_SAFE_INTEGER) - (b.lineStart ?? Number.MAX_SAFE_INTEGER) || compareStrings7(a.qualifiedName, b.qualifiedName));
}
...
function analyzeChanges(store, changedFilesInput, options = {}) {
  ...
  const reviewPriorities = [...nodeRisks].sort((a, b) => b.riskScore - a.riskScore || compareStrings7(a.qualifiedName, b.qualifiedName)).slice(0, 10);
  const summaryLines = [
...
function uniqueSorted5(values) {
  return [...new Set(values.map(normalize).filter(Boolean))].sort(compareStrings8);
}
...
function flowIdentifiers(values) {
  const result = /* @__PURE__ */ new Set();
  for (const value of values) {
...
function countHits(expected, actual) {
  return uniqueSorted5(expected ?? []).filter((item) => actual.has(item)).length;
}
function formatMetric2(value) {
  return value === null ? "n/a" : (value * 100).toFixed(1) + "%";
}
function evaluateReviewBenchmarks(store, cases, options = {}) {
  const results = cases.map((testCase) => {
    ...
    const impactedFiles = contextPayload && "reviewGuidance" in contextPayload ? uniqueSorted5(contextPayload.impactedFiles) : [];
    ...
    const expectedChangedNodes = uniqueSorted5(testCase.expectedChangedNodes ?? []);
    const expectedImpactedFiles = uniqueSorted5(testCase.expectedImpactedFiles ?? []);
    const expectedAffectedFlows = uniqueSorted5(testCase.expectedAffectedFlows ?? []);
    const expectedTestGaps = uniqueSorted5(testCase.expectedTestGaps ?? []);
    const expectedSummaryFacts = uniqueSorted5(testCase.expectedSummaryFacts ?? []);
    ...
    const changedFileSet = new Set(uniqueSorted5(testCase.changedFiles));
    const falsePositiveCount = impactedFiles.filter((file) => expectedImpactedFiles.length > 0 && !expectedImpactedFiles.includes(file) && !changedFileSet.has(file)).length;
    ...
    const estimatedTokenCount = estimateTokens3(summaryText);
    const tokenBudget = Math.max(1, testCase.tokenBudget ?? options.defaultTokenBudget ?? testCase.naiveTokenEstimate ?? 800);
    const impactedFilePrecision = ratio2(impactedHits, expectedImpactedFiles.length > 0 ? impactedFiles.length : 0);
    const impactedFileRecall = ratio2(impactedHits, expectedImpactedFiles.length);
    const metrics = {
      changedNodeRecall: ratio2(changedHits, expectedChangedNodes.length),
      impactedFilePrecision,
      impactedFileRecall,
      impactedFileF1: f1(impactedFilePrecision, impactedFileRecall),
      flowCompleteness: ratio2(flowHits, expectedAffectedFlows.length),
      testGapRecall: ratio2(testGapHits, expectedTestGaps.length),
      summaryFactRecall: ratio2(summaryHits, expectedSummaryFacts.length),
      falsePositiveCount,
      estimatedTokenCount,
      tokenBudget,
    ...
    return {
      name: testCase.name,
      changedFiles: uniqueSorted5(testCase.changedFiles),
      metrics,
      actual: {
        changedNodes: uniqueSorted5([...changedNodes]),
        impactedFiles,
        affectedFlows: uniqueSorted5([...affectedFlows]),
        testGaps: uniqueSorted5([...testGaps])
      },
      notes
    };
  });
  return {
    cases: results,
    aggregate: {
      changedNodeRecall: average2(results.map((item) => item.metrics.changedNodeRecall)),
      impactedFilePrecision: average2(results.map((item) => item.metrics.impactedFilePrecision)),
      impactedFileRecall: average2(results.map((item) => item.metrics.impactedFileRecall)),
      impactedFileF1: average2(results.map((item) => item.metrics.impactedFileF1)),
      flowCompleteness: average2(results.map((item) => item.metrics.flowCompleteness)),
      testGapRecall: average2(results.map((item) => item.metrics.testGapRecall)),
      summaryFactRecall: average2(results.map((item) => item.metrics.summaryFactRecall)),
...
function reviewBenchmarkToMarkdown(result) {
  const lines = [
    ...
    "## Aggregate",
    "",
    `- Changed-node recall: ${formatMetric2(result.aggregate.changedNodeRecall)}`,
    `- Impacted-file precision: ${formatMetric2(result.aggregate.impactedFilePrecision)}`,
    `- Impacted-file recall: ${formatMetric2(result.aggregate.impactedFileRecall)}`,
    `- Impacted-file F1: ${formatMetric2(result.aggregate.impactedFileF1)}`,
    `- Flow completeness: ${formatMetric2(result.aggregate.flowCompleteness)}`,
    `- Test-gap recall: ${formatMetric2(result.aggregate.testGapRecall)}`,
    `- Summary fact recall: ${formatMetric2(result.aggregate.summaryFactRecall)}`,
    `- False positives: ${result.aggregate.falsePositiveCount}`,
    `- Estimated graph-context tokens: ${result.aggregate.estimatedTokenCount}`,
    `- Token budget pass rate: ${formatMetric2(result.aggregate.tokenBudgetPassRate)}`,
    "",
    "## Cases",
    ""
  ];
  for (const testCase of result.cases) {
    lines.push(
      ...
      `- changed-node recall: ${formatMetric2(testCase.metrics.changedNodeRecall)}`,
      `- impacted-file precision: ${formatMetric2(testCase.metrics.impactedFilePrecision)}`,
      `- impacted-file recall: ${formatMetric2(testCase.metrics.impactedFileRecall)}`,
      `- flow completeness: ${formatMetric2(testCase.metrics.flowCompleteness)}`,
      `- test-gap recall: ${formatMetric2(testCase.metrics.testGapRecall)}`,
...
function uniqueSorted7(values) {
  return [...new Set([...values].filter(Boolean).map(normalizePath4))].sort(compareStrings10);
}
...
function fileInfo(G, file) {
  ...
  G.forEachNode((nodeId, attrs) => {
    const source = typeof attrs.source_file === "string" ? attrs.source_file : null;
    if (!sourceMatches3(source, file)) return;
    matched.push(nodeId);
    const community = maybeCommunity3(attrs.community);
    if (community !== null) communities.add(community);
  });
  matched.sort(compareStrings10);
  return {
    file,
...
function mergeDrafts(drafts, maxGroups) {
  ...
  const groups = [...merged.values()].map((group) => ({ ...group, files: uniqueSorted7(group.files) })).sort((a, b) => b.files.length - a.files.length || compareStrings10(a.title, b.title));
  ...
  kept.push({
    ...
    commitPrefix: "mixed",
    files: uniqueSorted7(overflow.flatMap((group) => group.files)),
...
function stalenessFrom(options) {
  ...
  const unique = uniqueSorted7(reasons);
  return {
...
function groupConfidence(group, delta, stale) {
  ...
  return { confidence, reasons: uniqueSorted7(reasons) };
}
...
function buildCommitRecommendation(G, changedFilesInput, options = {}) {
  const changedFiles = uniqueSorted7(changedFilesInput).filter((file) => !isGraphifyStatePath(file));
  ...
  return {
    ...
    staleness,
    confidence,
    confidence_reasons: [...confidenceReasons].sort(compareStrings10),
    groups,
...
import { copyFileSync, existsSync as existsSync18, mkdirSync as mkdirSync16, readdirSync as readdirSync7, statSync as statSync8 } from "fs";
import { dirname as dirname10, join as join17, relative as relative7, resolve as resolve20 } from "path";
init_paths();
...
function collectEntries(sourceDir, targetDir, force, base = "") {
  const entries = [];
  const items = readdirSync7(join17(sourceDir, base), { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name));
  for (const item of items) {
    const relativePath = base ? join17(base, item.name) : item.name;
    const source = join17(sourceDir, relativePath);
    const target = join17(targetDir, relativePath);
    const targetExists = existsSync18(target);
...
function gitAdvice(root, sourceDir, targetDir) {
  ...
  const legacyPath = normalizeGitPath(relative7(context.worktreeRoot, sourceDir));
  const targetPath = normalizeGitPath(relative7(context.worktreeRoot, targetDir));
...
function planGraphifyOutMigration(options = {}) {
  const root = resolve20(options.root ?? ".");
  const force = options.force === true;
  const paths = resolveGraphifyPaths({ root });
  const sourceDir = resolve20(root, LEGACY_GRAPHIFY_STATE_DIR);
  const targetDir = paths.stateDir;
  const sourceExists = existsSync18(sourceDir) && statSync8(sourceDir).isDirectory();
...
function applyEntry(entry) {
  ...
  if (entry.type === "directory") {
    mkdirSync16(entry.target, { recursive: true });
    return;
  }
  mkdirSync16(dirname10(entry.target), { recursive: true });
...
function migrateGraphifyOut(options = {}) {
  ...
  if (!options.dryRun && plan.sourceExists) {
    mkdirSync16(plan.targetDir, { recursive: true });
...
import { existsSync as existsSync19, readdirSync as readdirSync8, readFileSync as readFileSync19, statSync as statSync9 } from "fs";
import { basename as basename11, isAbsolute as isAbsolute2, relative as relative8, resolve as resolve21 } from "path";
var LOCAL_LIFECYCLE_FILES = /* @__PURE__ */ new Set([
  "branch.json",
  "worktree.json",
  "needs_update"
]);
var LOCAL_LIFECYCLE_PREFIXES = [
  ".graphify_",
  "cache/",
  "converted/",
  "memory/",
...
var TEXT_ARTIFACT_EXTENSIONS = /* @__PURE__ */ new Set([
  ".json",
  ".md",
  ".html",
  ".htm",
...
var EMBEDDED_PATH_PATTERN = /(^|[\s("'`:=])((?:\/(?:[A-Za-z0-9._-]+)[^\s"'`<>)\]]*)|(?:[A-Za-z]:[\\/][^\s"'`<>)\]]*)|(?:\.\.\/[^\s"'`<>)\]]*))/g;
...
function toProjectRelativePath(root, filePath) {
  ...
  const rootResolved = resolve21(root);
  const normalized = portablePath(filePath);
  if (isAbsolute2(filePath) || isWindowsAbsolutePath(filePath)) {
    return stripLeadingDotSlash(portablePath(relative8(rootResolved, resolve21(filePath))));
  }
  return stripLeadingDotSlash(portablePath(normalized));
}
function projectRootLabel(root) {
  const rel3 = relative8(process.cwd(), resolve21(root));
  if (!rel3) return ".";
  if (!rel3.startsWith("..")) return portablePath(rel3);
  return basename11(root);
}
...
function walkFiles2(root, dir, out) {
  for (const entry of readdirSync8(dir, { withFileTypes: true })) {
    const absolutePath = resolve21(dir, entry.name);
    const relativePath = portablePath(relative8(root, absolutePath));
...
function scanPortableGraphifyArtifacts(graphifyDir = ".graphify") {
  const root = resolve21(graphifyDir);
  const checkedFiles = [];
  const ignoredLocalFiles = [];
  const issues = [];
  if (!existsSync19(root) || !statSync9(root).isDirectory()) {
    return {
      ok: false,
      checkedFiles,
      ignoredLocalFiles,
  ...
  for (const relativePath of files.sort()) {
    ...
    const absolutePath = resolve21(root, relativePath);
    const content = readFileSync19(absolutePath, "utf-8");
...
import { basename as basename12, dirname as dirname11, resolve as resolve22 } from "path";
...
function loadGraph2(graphPath) {
  let safePath;
  try {
    safePath = validateGraphPath(graphPath, dirname11(resolve22(graphPath)));
  } catch (err) {
  ...
  let data;
  try {
    data = JSON.parse(readFileSync20(safePath, "utf-8"));
  } catch (err) {
    console.error(
...
function toolGetNeighbors(G, args) {
  ...
  forEachTraversalNeighbor(G, nid, (neighbor) => {
    ...
    if (!edgeKey) return;
    const d = G.getEdgeAttributes(edgeKey);
    const rel3 = d.relation ?? "";
    if (relFilter && !rel3.toLowerCase().includes(relFilter)) return;
    lines.push(
      `  --> ${G.getNodeAttribute(neighbor, "label") ?? neighbor} [${rel3}] [${d.confidence ?? ""}]`
    );
  });
  return lines.join("\n");
}
...
function toolShortestPath(G, args) {
  ...
  for (let i = 0; i < pathNodes.length - 1; i++) {
    ...
    segments.push(
      `--${rel3}${confStr}--> ${G.getNodeAttribute(v, "label") ?? v}`
    );
  }
...
async function serve(graphPath = resolveGraphInputPath(), transport) {
  ...
  const closed = new Promise((resolve23) => {
...
var isDirectExecution2 = typeof process !== "undefined" && typeof process.argv[1] === "string" && /^serve\.(?:js|mjs|cjs|ts)$/.test(basename12(process.argv[1]));
...
init_paths();
init_detect();
import { existsSync as existsSync20, mkdirSync as mkdirSync17, writeFileSync as writeFileSync16, unlinkSync as unlinkSync3 } from "fs";
import { resolve as pathResolve3, extname as extname13, basename as basename13 } from "path";
...
async function rebuildCode(watchPath, followSymlinks = false, options = {}) {
  try {
    ...
    const root = pathResolve3(watchPath);
    ...
    writeFileSync16(paths.scratch.detect, JSON.stringify(portableDetection, null, 2), "utf-8");
    if (portableDetection.scope) {
      writeFileSync16(paths.scope, JSON.stringify(portableDetection.scope, null, 2), "utf-8");
    }
    ...
    writeFileSync16(paths.report, report, "utf-8");
...
function notifyOnly(watchPath) {
  ...
  mkdirSync17(outDir, { recursive: true });
  const flagPath = paths.needsUpdate;
  writeFileSync16(flagPath, "1", "utf-8");
...
function hasNonCode(changedPaths) {
  return changedPaths.some((p) => !CODE_EXTENSIONS.has(extname13(p).toLowerCase()));
}
async function watch(watchPath, debounce = 3, options = {}) {
  ...
  const resolvedPath = pathResolve3(watchPath);
  let lastTrigger = 0;
  let pending = false;
...
var isDirectExecution3 = typeof process !== "undefined" && typeof process.argv[1] === "string" && /^watch\.(?:js|mjs|cjs|ts)$/.test(basename13(process.argv[1]));
...
export {
  ...
  assertValid,
  augmentDetectionWithPdfPreflight,
  ...
  markLifecycleStale,
  mergeGraphsFromFiles,
  migrateGraphifyOut,
  migrationResultToText,
  minimalContextToText,
  normalizeOntologyProfile,
  normalizeProjectConfig,
  normalizeRegistryRecord,
  parseOntologyProfile,
  parsePdfOcrMode2 as parsePdfOcrMode,
  parseProjectConfig,
  parseUnifiedDiff,
  pdfOcrSidecarStem,
  planGraphifyOutMigration,
  planLifecyclePrune,
  preflightLlmExecution,
  preflightPdf,
  prepareSemanticDetection,
  printBenchmark,
